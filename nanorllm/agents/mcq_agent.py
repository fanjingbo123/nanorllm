...
class MCQAgent(BaseAgent):
    """Agent that formats multiple-choice questions for the LLM.

    Observation may include:
      {"question": str, "choices": [str, ...]} or {"feedback": str}
    """
    ...
    def update_from_model(self, response):
        ...
        prompt_messages = self._messages.copy()
    ...
    def _format_observation(self, observation):
        if isinstance(observation, dict):
            ...
            q = observation.get("question")
            choices = observation.get("choices")
            if q is not None and isinstance(choices, list) and choices:
                lines = [str(q).strip(), "", "Options:"]
                # Enumerate with capital letters A, B, C, ...
                for i, ch in enumerate(choices):
                    letter = chr(ord('A') + i)
                    lines.append(f"  {letter}. {str(ch).strip()}")
                lines.append("")
                lines.append("Reply with ONLY the capital letter of your choice (e.g., A).")
                return "\n".join(lines)
            if q is not None:
                return q
